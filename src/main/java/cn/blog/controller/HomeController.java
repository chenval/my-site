...
//        this.blogBaseData(request,contentCond);//获取公共分类标签等数据
    ...
    @GetMapping(value = "/index/page/{p}")
    public String blogIndex2(
            HttpServletRequest request,
            @PathVariable("p")
                    int p,
            @RequestParam(value = "limit", required = false, defaultValue = "11")
        ...
        PageInfo<ContentDomain> archives = contentService.getArticlesByCond(contentCond, p, limit);
        request.setAttribute("archives", archives);//文章列表
        request.setAttribute("type", "archives");
        request.setAttribute("active", "blog");
//        this.blogBaseData(request, contentCond);//获取公共分类标签等数据
        return "site/index";
    }
